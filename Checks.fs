...
let getAppVersion () = 
    getParseResult (
        ...
        with err -> printfn "ERROR: Did not get App version due to:\n%s" err.Message; ""
    )
...
let checkSemVer appVer =
    ...
    | x when x.Success -> printfn "AppVer %s is a SemVer" appVer
    | _ -> printfn "ERROR: AppVer %s is not a SemVer" appVer
...
let getAllOntologies () =
    getParseResult (
        ...
        with err -> printfn "ERROR: Was not able to parse ontologies due to:\n%s" err.Message; ""
    )
...
let checkOnto onto =
    let checkDate () =
        ...
        with err -> printfn "ERROR: Ontology %s misses a System.DateTime in DateCreated" onto.Name
    checkDate ()